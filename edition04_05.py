...
class Ui_MainWindow(QtWidgets.QWidget):
    ...
    def sindata(self):
        wave=''
        self.time_now = datetime.datetime.now().strftime('%H:%M:%S.%f')
        pos, v, wave = sin_wave_time(self.sin_A, self.sin_F, self.sin_D, self.sin_D, self.time_start,
                                          self.time_now)
        temp = wave.split('\n')[0]
        data = temp + '<' + self.checksum(temp) + '>\r\n'
        # print(self.time_now+"---"+data)
        self.ser_motor2.senddata(data)
    ...
    def SinDataSend(self, state):
        if state == QtCore.Qt.Checked:
            self.time_start = datetime.datetime.now().strftime('%H:%M:%S.%f')
            # 发送指令
            # file_add = "D:\PY_Project\python_2022_8_3\sin_wave_cmd.txt"
            # self.wave = sin_wave_cmd(file_add, self.sin_A, self.sin_F, 50, self.sin_D, self.sin_B, 1/self.sin_F)
